...
!function ($) {
    ...
    var SpinEdit = function (element, options) {
        ...
        var aftertemplate = $(DRPGlobal.aftertemplate);
        var beforetemplate = $(DRPGlobal.beforetemplate);
        this.element.before(beforetemplate);
        this.element.after(aftertemplate);
	      $(aftertemplate).each(function (i,x) {
            $(x).bind('selectstart click mousedown', function () { return false; });
        });
        $(beforetemplate).each(function (i,x) {
            $(x).bind('selectstart click mousedown', function () { return false; });
        });
        aftertemplate.find('.icon-chevron-up').mousehold($.proxy(this.increase, this));
        beforetemplate.find('.icon-chevron-down').mousehold($.proxy(this.decrease, this));
    ...
    DRPGlobal.beforetemplate =
      '<span class="input-group-btn">' +
      '<a class="btn btn-default icon-chevron-down">-</span></a>' +
      '</span>'

    DRPGlobal.aftertemplate =
    '<span class="input-group-btn">' +
      '<a class="btn btn-default icon-chevron-up">+</span></a>' +
    '</span>'
